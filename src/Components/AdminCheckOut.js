...
function AdminCheckOut() {
    ...
    const fetchLocation = async () => {
        try {
            ...
            setAllReservation(locationData); // Store all locations for filtering purposes
        } catch (error) {
    ...
    const handleFilterChange = (event) => {
        ...
        if (value === "Available") {
            ...
            const fullSpacesLocations = allReservation.filter(
                (location) =>  Number(location.Capacity) >= Number(location.space)
            );
            setReservation(fullSpacesLocations);
        } else {
